...
import { entryBlocks, tabBlocks } from '../app/generated/tabs';
...
export function SectionsMenu({
    ...
    const baseData = variant === 'all' ? tabBlocks : entryBlocks;

    const data = useMemo(() => {
        const entryIds = new Set(entryBlocks.map((x) => x.id));
        return variant === 'all' && hideEntries
        ? baseData.filter((x) => !entryIds.has(x.id) && !String(x.route).endsWith('/menu'))
        : baseData;
    }, [baseData, hideEntries, variant]);
