import type { ImportanceAlternative, ImportanceCase, ImportanceReport } from '../types';
...
const gcd = (aRaw: number, bRaw: number): number => {
...
const lcm = (a: number, b: number) => Math.abs(a * b) / gcd(a, b || 1) || 1;

const approximateFraction = (value: number, maxDenominator = 12) => {
  let bestNum = 1;
  let bestDen = 1;
...
export function dominanceRelation(vecA: number[], vecB: number[]) {
  ...
  if (aGeB && aGtB) return 'dominates';
  if (bGeA && bGtA) return 'dominated';
  return 'incomparable';
}

export function validateImportanceCase(caseData: ImportanceCase) {
  ...
  if (criterionIds.size === 0) throw new Error('Нет критериев для анализа');
  if (alternativeIds.size < 2) throw new Error('Для анализа нужны минимум две альтернативы');

  for (const alternativeId of alternativeIds) {
    if (!caseData.scores[alternativeId]) throw new Error(`Нет оценок для альтернативы ${alternativeId}`);
    for (const criterionId of criterionIds) {
      const value = Number(caseData.scores[alternativeId][criterionId]);
      if (!Number.isFinite(value)) throw new Error(`Нет оценки для критерия ${criterionId} у альтернативы ${alternativeId}`);
    }
  }
  ...
  for (const relation of caseData.relations) {
    ...
    if (relation.op !== '>' && relation.op !== '=') throw new Error(`Неподдерживаемая операция: ${relation.op}`);
    if (!(Number(relation.factor) > 0)) throw new Error('Коэффициент важности должен быть положительным');
  }
}

const buildImportanceGroups = (caseData: ImportanceCase): ImportanceReport['analysisModel'] => {
  const criterionIds = caseData.criteria.map((criterion) => criterion.id);
  const dsu = new Dsu(criterionIds);

  for (const relation of caseData.relations) {
    if (relation.op === '=') dsu.union(relation.left, relation.right);
  }
  ...
  const edgeMap = new Map<string, { left: string; right: string; factor: number }>();
  for (const relation of caseData.relations) {
  ...
  const logWeights: Record<string, number> = Object.fromEntries(groupIds.map((id) => [id, 0]));
  ...
  const groupMultiplicity = Object.fromEntries(
    groupIds.map((id) => [id, Math.max(1, Math.trunc((fractions[id].num * commonDenominator) / fractions[id].den))])
  );
  const criterionMultiplicity = Object.fromEntries(
    criterionIds.map((criterionId) => [criterionId, groupMultiplicity[dsu.find(criterionId)]])
  );
...
const baseVectors = (caseData: ImportanceCase, criterionOrder: string[]) =>
  Object.fromEntries(
...
const expandedVectors = (
  caseData: ImportanceCase,
...
const dominanceMatrix = (alternatives: ImportanceAlternative[], vectors: Record<string, number[]>) =>
  Object.fromEntries(
...
const nondominatedSet = (alternatives: ImportanceAlternative[], dominance: Record<string, Record<string, string>>) =>
  alternatives
    .map((alternative) => alternative.id)
    .filter((id) => !alternatives.some((other) => other.id !== id && dominance[id][other.id] === 'dominated'));

const weightedSum = (caseData: ImportanceCase, criterionMultiplicity: Record<string, number>) =>
  Object.fromEntries(
    caseData.alternatives.map((alternative) => {
      const total = Object.entries(criterionMultiplicity).reduce(
        (sum, [criterionId, multiplicity]) => sum + Number(caseData.scores[alternative.id][criterionId]) * multiplicity,
        0
      );
...
const explain = (
  caseData: ImportanceCase,
  report: Pick<ImportanceReport, 'analysisModel' | 'finalNondominated' | 'weightedSum' | 'winnerId' | 'winnerName'>
) => {
  ...
  const winnerBest = Object.entries(winnerScores)
    .sort((a, b) =>
      Number(b[1]) * report.analysisModel.criterionMultiplicity[b[0]] -
...
export function runImportancePipeline(caseData: ImportanceCase): ImportanceReport {
  ...
  const ranking = [...caseData.alternatives]
    .sort((a, b) => {
      ...
      if (nondominatedDiff !== 0) return nondominatedDiff;
