...
import { getCategoryNameById, inferCapitalKindByText } from '../../../store/data/catalogRules';
import type {
  OptimizationCandidateItem,
  OptimizationCriteriaValues,
  OptimizationInput,
  OptimizationReport,
  OptimizationScope,
...
const normalizeWeights = (weights?: Partial<OptimizationWeights>): OptimizationWeights => {
...
const seededRandom = (seedRaw: number) => {
...
const scopeToKind = (scope: OptimizationScope): ItemKind | null => {
  if (scope === 'technical') return 'hardware';
  if (scope === 'software') return 'software';
  return null;
};

const hasClientRole = (item: CapitalEquipment, categoryName: string) => {
...
export function buildOptimizationCandidates(
  ...
  return items
    .map((item) => {
      ...
      const normalized: OptimizationCandidateItem = {
        ...item,
        quantity,
        price,
        kind,
...
const maskKey = (mask: number[]) => mask.join('');
const maskHasAny = (mask: number[]) => mask.some((bit) => bit === 1);

const selectedFromMask = (items: OptimizationCandidateItem[], mask: number[]) =>
  items.filter((_, index) => mask[index] === 1);

const uniqueCategoryIds = (items: OptimizationCandidateItem[]) =>
  Array.from(new Set(items.map((item) => item.categoryId))).sort();

const evaluateMask = (
  items: OptimizationCandidateItem[],
  mask: number[],
...
): Omit<OptimizationSolution, 'rank'> | null => {
  if (!maskHasAny(mask)) return null;
  ...
  if (maxBudget > 0 && totalCost > maxBudget) return null;
  if (requiredCategoryIds.some((categoryId) => !selectedCategorySet.has(categoryId))) return null;
  ...
  const criteria: OptimizationCriteriaValues = {
    costUtility,
    categoryCoverage,
  ...
  const score =
    criteria.costUtility * weights.cost +
...
const rankSolutions = (
  candidates: Array<Omit<OptimizationSolution, 'rank'>>,
...
const applyAhpRanking = (solutions: OptimizationSolution[], weights: OptimizationWeights) => {
...
export function runOptimization(input: OptimizationInput): OptimizationReport {
  const ga = {
  ...
  const archive = new Map<string, Omit<OptimizationSolution, 'rank'>>();
  const history = [];

  const registerMask = (maskRaw: number[]) => {
    const mask = repairMask(maskRaw, candidates, maxBudget, rand);
    const solution = evaluateMask(
      candidates,
      mask,
    ...
    if (!previous || solution.score > previous.score) archive.set(key, solution);
    return true;
  };

  const singletonPopulation = candidates.map((_, index) =>
    candidates.map((_item, itemIndex) => (itemIndex === index ? 1 : 0))
  );
  ...
  for (const index of greedyOrder) {
    greedyMask[index] = 1;
    if (!registerMask(greedyMask)) greedyMask[index] = 0;
  }
  ...
  const solutionScore = (mask: number[]) =>
    evaluateMask(candidates, mask, availableCategoryIds, requiredCategoryIds, maxBudget, targetClientSeats, weights)?.score ?? -1;
