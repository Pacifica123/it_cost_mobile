...
Object.defineProperty(exports, "initialState", { enumerable: true, get: function () { return defaults_2.initialState; } });
const nowIso = () => new Date().toISOString();
const MAX_EVENTS = 80;
const MAX_UNDO = 25;
const MAX_BACKUPS = 12;
const withMetaUpdate = (state) => ({
    ...state,
...
const withEvent = (state, title, description = '', type = 'data') => {
    const event = {
        ...(0, defaults_1.createProjectEvent)(title, description, type, nowIso()),
...
const withUndoPoint = (state) => {
    const current = (0, defaults_1.makeProjectSnapshot)(state);
    const last = state.undoStack[0];
    if (last && snapshotEquals(last, current))
...
const applySnapshot = (state, snapshot) => ({
    ...
    capitalData: (0, catalogRules_1.ensureCapitalKinds)(snapshot.capitalData, snapshot.categories),
    operatingData: snapshot.operatingData,
    categories: (0, catalogRules_1.ensureUniqueCategories)(snapshot.categories, snapshot.capitalData, snapshot.operatingData),
    electricityTotal: snapshot.electricityTotal,
});
const resetRuntimeStacks = (state) => ({
    ...state,
    schemaVersion: defaults_1.CURRENT_DATA_SCHEMA_VERSION,
    undoStack: [],
    redoStack: [],
});
const buildSavedProjectRecord = (state, input) => {
    const now = nowIso();
    const projectId = state.activeProjectId || `${Date.now().toString(36)}-project-${Math.random().toString(36).slice(2, 8)}`;
    const snapshot = (0, defaults_1.makeProjectSnapshot)(state);
    return {
        id: projectId,
...
function dataReducer(state, action) {
    switch (action.type) {
        ...
        case 'CREATE_PROJECT_BACKUP': {
            ...
            }, 'Создана резервная копия', backup.name, 'backup');
        }
        ...
        case 'RESET_EMPTY_PROJECT': {
            ...
            return withEvent({
                ...
                undoStack: undoState.undoStack,
                redoStack: [],
            }, 'Проект очищен', 'Позиции и расчёты очищены, базовые категории сохранены.', 'reset');
        }
        ...
        case 'SAVE_CURRENT_PROJECT': {
            ...
            }, 'Проект сохранён в список', record.name, 'project');
        }
        case 'OPEN_SAVED_PROJECT': {
            ...
            return withEvent({
                ...
                redoStack: [],
            }, 'Открыт сохранённый проект', record.name, 'project');
        }
        case 'DUPLICATE_SAVED_PROJECT': {
            ...
            }, 'Создана копия проекта', copy.name, 'project');
        }
        case 'DELETE_SAVED_PROJECT': {
            ...
            }, 'Удалён сохранённый проект', record.name, 'project');
        }
        ...
        case 'SET_CAPITAL_DATA': {
            const capitalData = (0, catalogRules_1.ensureCapitalKinds)(action.payload, state.categories);
            return withEvent({
                ...withUndoPoint(state),
                capitalData,
                categories: (0, catalogRules_1.ensureUniqueCategories)(state.categories, capitalData, state.operatingData),
            }, 'Изменены CAPEX-позиции', `Количество позиций: ${capitalData.length}.`, 'data');
        }
        case 'SET_OPERATING_DATA': {
            ...
            return withEvent({
                ...withUndoPoint(state),
                operatingData,
                categories: (0, catalogRules_1.ensureUniqueCategories)(state.categories, state.capitalData, operatingData),
            }, 'Изменены OPEX-позиции', `Количество позиций: ${operatingData.length}.`, 'data');
        }
        case 'SET_ELECTRICITY_TOTAL':
            return withEvent({ ...withUndoPoint(state), electricityTotal: action.payload }, 'Обновлено электропотребление', `Ежемесячная сумма: ${action.payload}.`, 'data');
        case 'ADD_CATEGORY': {
            const exists = state.categories.some((category) => category.scope === action.payload.scope &&
            ...
            }, 'Добавлена категория', action.payload.name, 'catalog');
        }
