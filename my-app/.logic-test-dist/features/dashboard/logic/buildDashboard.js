...
const categoryNameById = (state) => new Map(state.categories.map((category) => [category.id, category.name]));
const buildTopCapitalItems = (state, capitalTotal) => {
...
const makeStatusLabel = (readinessPercent, qualityScore, hasErrors) => {
    if (hasErrors)
        return 'нужно исправить ошибки';
    if (readinessPercent >= 90 && qualityScore >= 85)
        return 'готов к итоговой выгрузке';
    if (readinessPercent >= 70)
        return 'рабочий черновик';
