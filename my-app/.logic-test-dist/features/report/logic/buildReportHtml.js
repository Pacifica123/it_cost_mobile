...
const escapeHtml = (value) => String(value)
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#039;');
const renderInsight = (title, description) => `<li><strong>${escapeHtml(title)}.</strong> ${escapeHtml(description)}</li>`;
const renderGrouped = (title, grouped, getQty, getCost) => {
    const rows = Object.entries(grouped).flatMap(([category, items]) => [
        `<tr class="category"><td colspan="3">${escapeHtml(category)}</td></tr>`,
        ...items.map((item) => `<tr><td>${escapeHtml(item.name)}</td><td>${escapeHtml(getQty(item))}</td><td>${escapeHtml((0, currency_1.formatCurrencyRU)(getCost(item)))}</td></tr>`),
    ]).join('');
    return `
...
<body>
  <main>
    ...
    <section>
      ...
      <div class="grid">
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(meta?.budget ?? 0))}</div><div class="label">Бюджет</div></div>
        <div class="pill"><div class="value">${escapeHtml(meta?.targetClientSeats ?? 0)}</div><div class="label">Клиентские места</div></div>
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(report.grandTotalAnnual))}</div><div class="label">Итог за год</div></div>
      </div>
    ...
    <section>
      ...
      <div class="grid">
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(report.totalOneTimeExpenses))}</div><div class="label">Разовые затраты</div></div>
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(report.periodicTotalMonthly))}</div><div class="label">Периодические в месяц</div></div>
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(report.periodicTotalAnnual))}</div><div class="label">Периодические в год</div></div>
        <div class="pill"><div class="value">${escapeHtml((0, currency_1.formatCurrencyRU)(report.electricityTotalAnnual))}</div><div class="label">Электроэнергия в год</div></div>
      </div>
    </section>
    ...
    <section>
      ...
      <ul>
        <li>Техническое оборудование: ${escapeHtml((0, currency_1.formatCurrencyRU)(report.hardwareTotal))}</li>
        <li>Программное обеспечение: ${escapeHtml((0, currency_1.formatCurrencyRU)(report.softwareTotal))}</li>
        <li>OPEX и работы за год: ${escapeHtml((0, currency_1.formatCurrencyRU)(report.periodicTotalAnnual + report.oneTimeOperatingTotal))}</li>
        <li>Электроэнергия за год: ${escapeHtml((0, currency_1.formatCurrencyRU)(report.electricityTotalAnnual))}</li>
      </ul>
    </section>
