...
exports.buildProjectReadiness = buildProjectReadiness;
const selectors_1 = require("../../../store/data/selectors");
const includesClientWorkplace = (state) => {
    const categoryNameById = new Map(state.categories.map((category) => [category.id, category.name.toLowerCase()]));
    return state.capitalData.some((item) => {
        const categoryName = categoryNameById.get(item.categoryId) ?? '';
        const name = item.name.toLowerCase();
        return (categoryName.includes('клиент') ||
...
function buildProjectReadiness(state) {
    const hardwareTotal = sumCapitalByKind(state, 'hardware');
    const softwareTotal = sumCapitalByKind(state, 'software');
    const periodicOpex = (0, selectors_1.selectOperatingByMode)(state, 'periodic');
    const oneTimeOpex = (0, selectors_1.selectOperatingByMode)(state, 'oneTime');
