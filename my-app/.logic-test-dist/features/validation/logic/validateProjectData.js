...
const categoryNameById = (state, categoryId) => state.categories.find((category) => category.id === categoryId)?.name ?? '';
const itemLooksLike = (name, pattern) => pattern.test(name.toLowerCase());
function validateProjectData(state) {
    const issues = [];
    ...
    const capitalNameCounts = new Map();
    ...
    const operatingNameCounts = new Map();
